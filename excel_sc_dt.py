...
known_image = face_recognition.load_image_file("shreyash.jpg")
...
cam = cv.VideoCapture("shreyash.mp4")
if not cam.isOpened():
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"screenshots/Shreyash_Wadgaonkar_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                ...
                for face_location in face_locations:
                    ...
                    cv.putText(frame, 'Shreyash Wadgaonkar', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Barack Obama'
            cv.putText(frame, 'Not Shreyash Wadgaonkar', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
